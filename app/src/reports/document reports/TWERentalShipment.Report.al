/// <summary>
/// Report TWE Rental - Shipment (ID 50007).
/// </summary>
report 50007 "TWE Rental - Shipment"
{
    RDLCLayout = 'RentalShipment.rdlc';
    //WordLayout = './StandardSalesShipment.docx';
    Caption = 'Rental - Shipment';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "TWE Rental Shipment Header")
        {
            DataItemTableView = SORTING("No.");
            RequestFilterFields = "No.", "Rented-to Customer No.", "No. Printed";
            RequestFilterHeading = 'Posted Rental Shipment';
            ...
            dataitem(Line; "TWE Rental Shipment Line")
            {
    ...
    var
        ...
        RentalShipmentLbl: Label 'Shipment';
    ...
    local procedure FormatAddressFields(var RentalShipmentHeader: Record "TWE Rental Shipment Header")
    begin
        RentalFormatAddr.GetCompanyAddr(RentalShipmentHeader."Responsibility Center", RespCenter, CompanyInfo, CompanyAddr);
        RentalFormatAddr.RentalShptBillTo(CustAddr, ShipToAddr, RentalShipmentHeader);
        RentalFormatAddr.RentalShptShipTo(ShipToAddr, RentalShipmentHeader);
        ShowCustAddr := RentalFormatAddr.RentalShptBillTo(CustAddr, ShipToAddr, RentalShipmentHeader);
    end;

    local procedure FormatDocumentFields(RentalShipmentHeader: Record "TWE Rental Shipment Header")
    begin
        RentalFormatDocument.SetSalesPerson(SalespersonPurchaser, RentalShipmentHeader."Salesperson Code", SalesPersonText);
        RentalFormatDocument.SetShipmentMethod(ShipmentMethod, RentalShipmentHeader."Shipment Method Code", RentalShipmentHeader."Language Code");
    end;
    ...
    local procedure OnAfterLineOnPreDataItem(var RentalShipmentHeader: Record "TWE Rental Shipment Header"; var RentalShipmentLine: Record "TWE Rental Shipment Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeLineOnAfterGetRecord(var RentalShipmentHeader: Record "TWE Rental Shipment Header"; var RentalShipmentLine: Record "TWE Rental Shipment Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetDocumentCaption(RentalShipmentHeader: Record "TWE Rental Shipment Header"; var DocCaption: Text)
    begin
    end;

    [IntegrationEvent(TRUE, FALSE)]
    local procedure OnAfterGetRentalHeader(RentalShipmentHeader: Record "TWE Rental Shipment Header")
    begin
    end;
}
