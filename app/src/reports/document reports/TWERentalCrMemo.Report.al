/// <summary>
/// Report TWE Rental Cr. Memo (ID 50002).
/// </summary>
report 50002 "TWE Rental Cr. Memo"
{
    RDLCLayout = 'RentalCrMemo.rdlc';
    //WordLayout = 'RentalCrMemo.docx';
    Caption = 'Rental - Cr. Memo';
    DefaultLayout = RDLC;
    EnableHyperlinks = true;
    Permissions = TableData "TWE Rental Shipment Buffer" = rimd;
    PreviewMode = PrintLayout;
    //WordMergeDataItem = Header;

    dataset
    {
        dataitem(Header; "TWE Rental Cr.Memo Header")
        {
            DataItemTableView = SORTING("No.");
            RequestFilterFields = "No.", "Rented-to Customer No.", "No. Printed";
            RequestFilterHeading = 'Posted Rental Cr. Memo';
            ...
            column(AddressLine; GetAddressLine(CompanyInfo))
            {
            }
            ...
            column(CustomerPostalBarCode; RentalFormatAddr.PostalBarCode(1))
            {
            }
            ...
            column(RentedToContactPhoneNoLbl; RentedToContactPhoneNoLbl)
            {
            }
            column(RentedToContactMobilePhoneNoLbl; RentedToContactMobilePhoneNoLbl)
            {
            }
            column(RentedToContactEmailLbl; RentedToContactEmailLbl)
            {
            }
            ...
            column(RentedToContactPhoneNo; RentedToContact."Phone No.")
            {
            }
            column(RentedToContactMobilePhoneNo; RentedToContact."Mobile Phone No.")
            {
            }
            column(RentedToContactEmail; RentedToContact."E-Mail")
            {
            }
            ...
            column(RentedtoCustomerNo_Lbl; FieldCaption("Rented-to Customer No."))
            {
            }
            ...
            column(RentedToPhoneNo; "Rented-to Phone No.")
            {
            }
            ...
            column(RentalInvoiceLineDiscount_Lbl; RentalInvLineDiscLbl)
            {
            }
            ...
            column(YourDocumentTitle_Lbl; YourRentalInvoiceLbl)
            {
            }
            ...
            dataitem(Line; "TWE Rental Cr.Memo Line")
            {
                ...
                column(Total_Days_to_Invoice; "Invoiced Duration")
                {
                }
                column(Total_Days_to_Invoice_Lbl; Total_Days_to_Invoice_Lbl)
                {
                }
                dataitem(ShipmentLine; "TWE Rental Shipment Buffer")
                {
                ...
                dataitem(AssemblyLine; "Posted Assembly Line")
                {
                    ...
                    begin
                        ...
                        GetAssemblyLinesForDocument(
                          AssemblyLine, ValueEntry."Document Type"::"Rental Invoice", Line."Document No.", Line."Line No.");
                    end;
                }

                trigger OnAfterGetRecord()
                var
                    Placeholder001Lbl: Label '%1%', Comment = '%1 = Line Discount %';
                begin
                    ...
                    if "Line Discount %" = 0 then
                        ...
                        LineDiscountPctText := StrSubstNo(Placeholder001Lbl, -Round("Line Discount %", 0.1));
                    ...
                    RentalFormatDocument.SetRentalCrMemoLine(Line, FormattedQuantity, FormattedUnitPrice, FormattedVATPct, FormattedLineAmount);
                end;
            ...
            begin
                ...
                if not IsReportInPreviewMode() then
                    CODEUNIT.Run(CODEUNIT::"TWE Rental Inv.-Printed", Header);
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                ...
                {
                    Caption = 'Options';
                    field(LogInteraction; LogInteractionBoolean)
                    {
                    ...
                    field(DisplayShipmentInformation; DisplayShipmentInformationBoolean)
                    {
                        ApplicationArea = Basic, Suite;
        ...
        begin
            InitLogInteraction();
            LogInteractionEnable := LogInteractionBoolean;
        end;
    }
    ...
    begin
        ...
        CompanyLogoPosition := RentalSetup."Logo Position on Documents";
    end;
    ...
    var
        ...
        DisplayShipmentInformationBoolean: Boolean;
        ...
        RentalInvLineDiscLbl: Label 'Discount %';
        RentalInvoiceLbl: Label 'Cr. Memo';
        YourRentalInvoiceLbl: Label 'Your Cr. Memo:';
        ...
        RentedToContactPhoneNoLbl: Label 'Rented-to Contact Phone No.';
        RentedToContactMobilePhoneNoLbl: Label 'Rented-to Contact Mobile Phone No.';
        RentedToContactEmailLbl: Label 'Rented-to Contact E-Mail';
        ...
        Total_Days_to_Invoice_Lbl: Label 'Invoiced Days';
        GreetingLbl: Label 'Hello';
        ClosingLbl: Label 'Sincerely';
        PmtDiscTxt: Label 'If we receive the payment before %1, you are eligible for a 2% payment discount.', Comment = '%1 = Discount Due Date %2 = value of Payment Discount Percent ';
    ...
    var
        RentalShipmentBuffer2: Record "TWE Rental Shipment Buffer";
    begin
        ...
        if ShipmentLine.FindFirst() then begin
            RentalShipmentBuffer2 := ShipmentLine;
            if not DisplayShipmentInformationBoolean then
                if ShipmentLine.Next() = 0 then begin
                    ShipmentLine.Get(RentalShipmentBuffer2."Document No.", RentalShipmentBuffer2."Line No.", RentalShipmentBuffer2."Entry No.");
                    ShipmentLine.Delete();
                    exit;
                end;
    ...
    begin
        LogInteractionBoolean := NewLogInteraction;
    ...
    local procedure FormatAddressFields(var RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header")
    begin
        RentalFormatAddr.GetCompanyAddr(RentalCrMemoHeader."Responsibility Center", RespCenter, CompanyInfo, CompanyAddr);
        RentalFormatAddr.RentalCrMemoBillTo(CustAddr, RentalCrMemoHeader);
        ShowShippingAddr := RentalFormatAddr.RentalCrMemoShipTo(ShipToAddr, CustAddr, RentalCrMemoHeader);
    end;

    local procedure FormatDocumentFields(RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header")
    var
        Placeholder001Lbl: Label '%1 %2', Comment = '%1 = Applies-to Doc. Type ,%2 = Applies-to Doc. No.';
    begin
        RentalFormatDocument.SetTotalLabels(RentalCrMemoHeader."Currency Code", TotalText, TotalInclVATText, TotalExclVATText);
        RentalFormatDocument.SetSalesPerson(SalespersonPurchaser, RentalCrMemoHeader."Salesperson Code", SalesPersonText);
        RentalFormatDocument.SetPaymentTerms(PaymentTerms, RentalCrMemoHeader."Payment Terms Code", RentalCrMemoHeader."Language Code");
        RentalFormatDocument.SetPaymentMethod(PaymentMethod, RentalCrMemoHeader."Payment Method Code", RentalCrMemoHeader."Language Code");
        RentalFormatDocument.SetShipmentMethod(ShipmentMethod, RentalCrMemoHeader."Shipment Method Code", RentalCrMemoHeader."Language Code");

        AppliesToText :=
          RentalFormatDocument.SetText(RentalCrMemoHeader."Applies-to Doc. No." <> '', StrSubstNo(Placeholder001Lbl, Format(RentalCrMemoHeader."Applies-to Doc. Type"), RentalCrMemoHeader."Applies-to Doc. No."));
    end;
    ...
    local procedure OnAfterLineOnPreDataItem(var RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header"; var RentalCrMemoLine: Record "TWE Rental Cr.Memo Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeLineOnAfterGetRecord(var RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header"; var RentalCrMemoLine: Record "TWE Rental Cr.Memo Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetDocumentCaption(RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header"; var DocCaption: Text)
    begin
    end;

    [IntegrationEvent(TRUE, FALSE)]
    local procedure OnAfterGetRentalHeader(RentalCrMemoHeader: Record "TWE Rental Cr.Memo Header")
    begin
    end;
