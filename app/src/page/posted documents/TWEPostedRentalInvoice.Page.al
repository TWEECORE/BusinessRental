/// <summary>
/// Page TWE Posted Rental Invoice (ID 70704625).
/// </summary>
page 50002 "TWE Posted Rental Invoice"
{
        ...
        area(content)
        {
            group(General)
            {
                ...
                field("Order No."; Rec."Order No.")
                {
                    ...
                    Editable = false;
                    Importance = Promoted;
                    ToolTip = 'Specifies the number of the rental order that this invoice was posted from.';
                }
                field("Pre-Assigned No."; Rec."Pre-Assigned No.")
                {
                    ...
                    Editable = false;
                    Importance = Additional;
                    ToolTip = 'Specifies the number of the rental document that the posted invoice was created for.';
                }
                ...
                field("Responsibility Center"; Rec."Responsibility Center")
                {
                    ...
                    Editable = false;
                    Importance = Additional;
                    ToolTip = 'Specifies the code of the responsibility center associated with the user who created the invoice, your company, or the customer in the rental invoice.';
                }
            ...
            part(RentalInvLines; "TWE Posted Rental Inv. Subform")
            {
                ApplicationArea = Basic, Suite;
            ...
            group("Invoice Details")
            {
                ...
                field("Payment Terms Code"; Rec."Payment Terms Code")
                {
                    ...
                    Editable = false;
                    Importance = Promoted;
                    ToolTip = 'Specifies a formula that calculates the payment due date, payment discount date, and payment discount amount on the rental document.';
                }
                field("Payment Method Code"; Rec."Payment Method Code")
                {
                    ...
                    Editable = false;
                    Importance = Additional;
                    ToolTip = 'Specifies how the customer must pay for products on the rental document.';
                }
                group(Control15)
                {
                    ShowCaption = false;
                    ...
                    field(SelectedPayments; Rec.GetSelectedPaymentsText())
                    {
                        ...
                        MultiLine = true;
                        ToolTip = 'Specifies the payment service, such as PayPal, that the rental invoice can be paid with.';
            ...
            group("Foreign Trade")
            {
                ...
                field("Transport Method"; Rec."Transport Method")
                {
                    ApplicationArea = BasicEU;
                    Editable = false;
                    ToolTip = 'Specifies the transport method of the rental header that this line was posted from.';
                }
        ...
        area(navigation)
        {
            group("&Invoice")
            {
                ...
                /* action(ChangePaymentService)
                {
                    ...
                    PromotedIsBig = true;
                    PromotedOnly = true;
                    ToolTip = 'Change or add the payment service, such as PayPal Standard, that will be included on the rental document so the customer can quickly access the payment site.';
                    Visible = PaymentServiceVisible;
        ...
        area(processing)
        {
            ...
            action(Print)
            {
                ...
                begin
                    ...
                    CurrPage.SetSelectionFilter(RentalInvHeader);
                    IsHandled := false;
                    OnBeforeRentalInvHeaderPrintRecords(RentalInvHeader, IsHandled);
            ...
            action(AttachAsPDF)
            {
                ...
                begin
                    RentalInvoiceHeader := Rec;
                    RentalInvoiceHeader.SetRecFilter();
                    Rec.PrintToDocumentAttachment(RentalInvoiceHeader);
                end;
            }
            action("&Navigate")
            {
            ...
            group(Correct)
            {
                ...
                action(CorrectInvoice)
                {
                    ...
                    trigger OnAction()
                    var
                        CorrectPstdRentalInvYesNo: Codeunit "TWE Corr.PstdRentalInv(Yes/No)";
                    begin
                        if CorrectPstdRentalInvYesNo.CorrectInvoice(Rec) then
                            CurrPage.Close();
                    end;
                }
                action(CancelInvoice)
                {
                    ...
                    trigger OnAction()
                    var
                        CancelPstdRentalInvYesNo: Codeunit "TWE CancelPstdRentInv(Yes/No)";
                    begin
                        if CancelPstdRentalInvYesNo.CancelInvoice(Rec) then
                            CurrPage.Close();
                    end;
                }
                action(CreateCreditMemo)
                {
                    ...
                    begin
                        if CorrectPostedRentalInvoice.CreateCreditMemoCopyDocument(Rec, RentalHeader) then begin
    ...
    local procedure OnBeforeRentalInvHeaderPrintRecords(var RentalInvHeader: Record "TWE Rental Invoice Header"; var IsHandled: Boolean)
    begin
    end;
}
