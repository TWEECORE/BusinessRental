/// <summary>
/// Page TWE Post. Rental Return Ship. (ID 50009).
/// </summary>
page 50009 "TWE Post. Rental Return Ship."
{
    Caption = 'Posted Rental Return Shipment';
    InsertAllowed = false;
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            ...
            part(RentalReturnShipmLines; "TWE Pstd. Ret. Rent. Shpt. SF")
            {
                ApplicationArea = Basic, Suite;
    ...
    actions
    {
        ...
        area(processing)
        {
            action("&Print")
            {
                ...
                trigger OnAction()
                begin
                    RentalReturnShptHeader := Rec;
                    OnBeforePrintRecords(Rec, RentalReturnShptHeader);
                    CurrPage.SetSelectionFilter(RentalReturnShptHeader);
                    RentalReturnShptHeader.PrintRecords(true);
                end;
            }
            action("&Navigate")
            {
    ...
    local procedure OnBeforePrintRecords(RentalReturnShptHeaderRec: Record "TWE Rental Return Ship. Header"; var RentalReturnShptHeaderToPrint: Record "TWE Rental Return Ship. Header")
    begin
    end;
}
