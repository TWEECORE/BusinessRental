/// <summary>
/// Table TWE Rental Item Unit Of Measure (ID 707064602).
/// </summary>
table 50015 "TWE Rental Item Unit Of Meas."
{
    Caption = 'Rental Item Unit of Measure';
    LookupPageID = "TWE Rental Item Units of Mes.";
    ...
    var
        ...
        Text000LblTxt: Label 'must be greater than 0';
        Text001LblTxt: Label 'You cannot rename %1 %2 for item %3 because it is the item''s %4 and there are one or more open ledger entries for the MainRentalItem.', Comment = ' %1 TableCaption, %2 xRec.Code, %3 "Item No.", %4 FieldCaption("Base Unit of Measure"))';
        ...
        CannotModifyUOMWithWhseEntriesErr: Label 'You cannot modify %1 %2 for item %3 because there are one or more warehouse adjustment entries for the MainRentalItem.', Comment = '%1 = Item Unit of Measure %2 = Code %3 = Item No.';
    ...
    begin
        ...
        Weight := "Qty. per Unit of Measure" * MainRentalItem."Net Weight";
    ...
    begin
        if localMainRentalItem.Get("Item No.") then
            if localMainRentalItem."Base Unit of Measure" = xRec.Code then begin
                ItemLedgEntry.SetCurrentKey("Item No.", Open);
                ItemLedgEntry.SetRange("Item No.", "Item No.");
                ItemLedgEntry.SetRange(Open, true);
                if not ItemLedgEntry.IsEmpty then
                    Error(Text001LblTxt, TableCaption, xRec.Code, "Item No.", localMainRentalItem.FieldCaption("Base Unit of Measure"));
            end;
    end;
    ...
    begin
        if MainRentalItem.Get("Item No.") then begin
            if MainRentalItem."Base Unit of Measure" = xRec.Code then
                Error(CannotModifyBaseUnitOfMeasureErr, xRec.Code, "Item No.");
            if MainRentalItem."Sales Unit of Measure" = xRec.Code then
                Error(CannotModifySalesUnitOfMeasureErr, xRec.Code, "Item No.");
            if MainRentalItem."Purch. Unit of Measure" = xRec.Code then
    ...
    local procedure OnAfterCalcCubage(var ItemUnitOfMeasure: Record "TWE Rental Item Unit Of Meas.")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCalcWeight(var ItemUnitOfMeasure: Record "TWE Rental Item Unit Of Meas.")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterTestItemSetup(var Rec: Record "TWE Rental Item Unit Of Meas."; xRec: Record "TWE Rental Item Unit Of Meas.")
    begin
    end;
}
