/// <summary>
/// Table TWE Rental Invoice Line (ID 50011).
/// </summary>
table 50011 "TWE Rental Invoice Line"
{
    Caption = 'Rental Invoice Line';
    DrillDownPageID = "TWE Post. Rental Invoice Lines";
    LookupPageID = "TWE Post. Rental Invoice Lines";
    ...
    fields
    {
        ...
        field(6; "No."; Code[20])
        {
            ...
            ELSE
            IF (Type = CONST("Rental Item")) Item
            ELSE
    ...
    keys
    {
        ...
        key(Key4; "Rented-to Customer No.", Type, "Document No.")
        {
            Enabled = false;
    ...
    begin
        RentalDocLineComments.SetRange("Document Type", RentalDocLineComments."Document Type"::"Posted Invoice");
        ...
        PostedDeferralHeader.DeleteHeader(
            ...
            RentalDocLineComments."Document Type"::"Posted Invoice".AsInteger(), "Document No.", "Line No.");
    end;
    ...
    begin
        GetHeader();
        exit(RentalInvoiceHeader."Currency Code");
    end;
    ...
    /// CalcVATAmountLines.
    /// </summary>
    /// <param name="RentalInvHeader">Record "TWE Rental Invoice Header".</param>
    /// <param name="TempVATAmountLine">Temporary VAR Record "VAT Amount Line".</param>
    procedure CalcVATAmountLines(RentalInvHeader: Record "TWE Rental Invoice Header"; var TempVATAmountLine: Record "VAT Amount Line" temporary)
    begin
        TempVATAmountLine.DeleteAll();
        SetRange("Document No.", RentalInvHeader."No.");
        if Find('-') then
            repeat
    ...
    begin
        GetHeader();
        if not RentalInvoiceHeader."Prices Including VAT" then
            exit("Line Amount");
    ...
    begin
        GetHeader();
        if RentalInvoiceHeader."Prices Including VAT" then
            exit("Line Amount");
    ...
    begin
        if RentalInvoiceHeader."No." = "Document No." then
            exit;
        if not RentalInvoiceHeader.Get("Document No.") then
            RentalInvoiceHeader.Init();

        if RentalInvoiceHeader."Currency Code" = '' then
            Currency.InitRoundingPrecision()
        else
            if not Currency.Get(RentalInvoiceHeader."Currency Code") then
                Currency.InitRoundingPrecision();
    end;
    ...
    var
        "Field": Record "Field";
    begin
        Field.Get(DATABASE::"TWE Rental Invoice Line", FieldNumber);
    ...
    var
        Placeholder001Lbl: Label '3,%1', Comment = '%1 = Fieldnumber Fieldcaption';
    begin
        GetHeader();
        case FieldNumber of
            FieldNo("No."):
    ...
    /// GetRentalShptLines.
    /// </summary>
    /// <param name="TempRentalShptLine">Temporary VAR Record "TWE Rental Shipment Line".</param>
    procedure GetRentalShptLines(var TempRentalShptLine: Record "TWE Rental Shipment Line" temporary)
    var
        ...
        if ValueEntry.FindSet() then
            repeat
                ...
                if ItemLedgEntry."Document Type" = ItemLedgEntry."Document Type"::"Sales Shipment" then
                    if RentalShptLine.Get(ItemLedgEntry."Document No.", ItemLedgEntry."Document Line No.") then begin
                        TempRentalShptLine.Init();
    ...
    begin
        if Type = Type::"Rental Item" then begin
            GetRentalShptLines(TempRentalShptLine);
            PAGE.RunModal(0, TempRentalShptLine);
        end;
    end;
    ...
    begin
        RentalCommentLine.ShowComments(RentalCommentLine."Document Type"::"Posted Invoice".AsInteger(), "Document No.", "Line No.");
    end;
    ...
    procedure InitFromRentalLine(RentalInvHeader: Record "TWE Rental Invoice Header"; RentalLine: Record "TWE Rental Line")
    begin
        Init();
        TransferFields(RentalLine);
        if ("No." = '') and HasTypeToFillMandatoryFields() then
            Type := Type::" ";
        "Posting Date" := RentalInvHeader."Posting Date";
        "Document No." := RentalInvHeader."No.";
        ...
        "Line Amount" := RentalLine.GetLineAmountToInvoice();

        OnAfterInitFromRentalLine(Rec, RentalInvHeader, RentalLine);
    end;
        ...
        if Type in [Type::"Rental Item", Type::Resource] then
            if "Line Discount %" = 0 then
                "Price description" := StrSubstNo(
                    PriceDescriptionTxt, Quantity, localCurrency.ResolveGLCurrencySymbol(GetCurrencyCode()),
                    "Unit Price", "Unit of Measure")
            else
                "Price description" := StrSubstNo(
                    PriceDescriptionWithLineDiscountTxt, Quantity, localCurrency.ResolveGLCurrencySymbol(GetCurrencyCode()),
    ...
    var
        RentalCommentLine: Record "TWE Rental Comment Line";
    begin
        exit(RentalCommentLine."Document Type"::"Posted Invoice".AsInteger())
    end;
    ...
    begin
        exit(Type in [Type::" ", Type::"Rental Item", Type::"G/L Account"]);
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInitFromRentalLine(var RentalInvLine: Record "TWE Rental Invoice Line"; RentalInvHeader: Record "TWE Rental Invoice Header"; RentalLine: Record "TWE Rental Line")
    begin
    end;
