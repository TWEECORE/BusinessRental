/// <summary>
/// Table TWE Rental Shipment Header (ID 50032).
/// </summary>
table 50032 "TWE Rental Shipment Header"
{
    Caption = 'Rental Shipment Header';
    DataCaptionFields = "No.", "Rented-to Customer Name";
    DrillDownPageID = "TWE Posted Rental Shipments";
    LookupPageID = "TWE Posted Rental Shipments";
    ...
    begin
        ...
        RentalCommentLine.SetRange("Document Type", RentalCommentLine."Document Type"::Shipment);
    ...
    begin
        IsHandled := false;
        OnBeforeSendProfile(RentalDocumentSendingProfile, Rec, IsHandled);
        if IsHandled then
            exit;

        RentalDocumentSendingProfile.Send(
          DummyRentalReportSelections.Usage::"R.Shipment".AsInteger(), Rec, "No.", "Rented-to Customer No.",
    ...
    var
        ...
        IsHandled: Boolean;
    begin
        RentalShptHeader.Copy(Rec);
        OnBeforePrintRecords(RentalShptHeader, ShowRequestForm, IsHandled);
        if IsHandled then
            exit;

        RentalReportSelection.PrintWithDialogForCust(
          RentalReportSelection.Usage::"R.Shipment", RentalShptHeader, ShowRequestForm, FieldNo("Bill-to Customer No."));
    end;
    ...
    begin
        ...
        RentalDocumentSendingProfile.TrySendToEMail(
          DummyRentalReportSelections.Usage::"R.Shipment".AsInteger(), Rec, FieldNo("No."),
    ...
    local procedure OnBeforeEmailRecords(var RentalShipmentHeader: Record "TWE Rental Shipment Header"; SendDialog: Boolean; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforePrintRecords(var RentalShipmentHeader: Record "TWE Rental Shipment Header"; ShowDialog: Boolean; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeSendProfile(var RentalDocumentSendingProfile: Record "TWE Rental Doc. Send. Profile"; var RentalShipmentHeader: Record "TWE Rental Shipment Header"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnLookupAppliesToDocNoOnAfterSetFilters(var CustLedgEntry: Record "Cust. Ledger Entry"; RentalShipmentHeader: Record "TWE Rental Shipment Header")
    begin
    end;
}
